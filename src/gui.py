...
class Window(tkinter.Frame):
    ...
    def __init__(self, master, app):
        ...
        for name in app.classifier:
            tkinter.Radiobutton(self.modes, text=name, variable=self.mode,
                                value=name).grid(row=i, sticky=tkinter.W)
            i += 1
    ...
    def print_matrix(self, img, label="_"):
        ...
        for i in range(28):
            for j in range(28):
                if type(img) == list:
                    pi.put("#%02x%02x%02x" % (int(
                        255 * (1 - img[i * 28 + j])), int(255 * (1 - img[i * 28 + j])), int(255 * (1 - img[i * 28 + j]))), (j, i))
                else:
                    pi.put("#%02x%02x%02x" %
                           (255 * img[i, j], 255 * img[i, j], 255 * img[i, j]), (i, j))
        pi = pi.zoom(8, 8)
        self.pi = pi
    ...
    def create_widgets(self):
        ...
        self.canvas = tkinter.Canvas(self.left, width=self.CANVAS_WIDTH,
        ...
        tkinter.Button(self.center, text='Chiffre aléatoire',
                       command=self.app.test).grid(row=1, sticky=tkinter.EW)
        tkinter.Button(self.center, text='Entrainer',
                       command=self.app.train).grid(row=2, sticky=tkinter.EW)
        tkinter.Button(self.center, text='Charger', command=self.menu_load_classifier).grid(
            row=3, sticky=tkinter.EW)
        ...
        tkinter.Checkbutton(self.sidebar, text='Tester', var=self.testing,
                            command=self.app.init_test).grid(row=1, sticky=tkinter.W)
    ...
    def motion(self, event):
        ...
        if self.b1down:
            size = 6
            self.canvas.create_oval(self.lastx - size, self.lasty - size,
                                    self.lastx + size, self.lasty + size, fill='black', width=0)
            self.imdraw.ellipse([self.lastx - 8, self.lasty - size, self.lastx +
                                 size, self.lasty + size], fill='black', outline=None)
